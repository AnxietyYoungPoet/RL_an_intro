...
class State:
  ...
  # compute the hash value for one state, it's unique. 3-nary
  def hash(self):
  ...
  def is_end(self):
    ...
    # whether it's a tie
    chess_sum = np.sum(np.abs(self.data))
    if chess_sum == BOARD_ROWS * BOARD_COLS:
      self.winner = 0
      self.end = True
      return self.end
