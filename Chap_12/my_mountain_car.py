...
def transition(state, action):
  ...
  new_position = np.clip(position + new_velocity, POSITION_MIN, POSITION_MAX)
  reward = -1.0
  if new_position == POSITION_MIN:
    new_velocity = 0.
  return reward, (new_position, new_velocity)
...
# replacing trace update rule, 'clearing' means set
# all tiles corresponding to non-selected actions to 0
# @trace: old trace (will be modified)
# @activeTiles: current active tile indices
# @lam: lambda
...
def get_action(state, valueFunction):
  if np.random.binomial(1, EPSILON) == 1:
    return np.random.choice(ACTIONS)
  values = []
  for action in ACTIONS:
    values.append(valueFunction.value(state, action))
...
def play(evaluator):
  ...
  action = get_action(state, evaluator)
  steps = 0
  while True:
    reward, new_state = transition(state, action)
    next_action = get_action(new_state, evaluator)
    steps += 1
    target = reward + DISCOUNT * \
        evaluator.value(new_state, next_action)
...
def figure_12_11():
  traceTypes = [dutch_trace, replacing_trace,
                replacing_trace_with_clearing, accumulating_trace]
