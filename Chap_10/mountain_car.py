...
class ValueFunction:
    ...
    def __init__(self, step_size, num_of_tilings=8, max_size=2048):
        ...
        self.position_scale = self.num_of_tilings / \
            (POSITION_MAX - POSITION_MIN)
        self.velocity_scale = self.num_of_tilings / \
...
def get_action(position, velocity, value_function):
    ...
    return np.random.choice(
        [action_ for action_, value_ in enumerate(values) if value_ == np.max(values)]) - 1
...
def semi_gradient_n_step_sarsa(value_function, n=1):
    ...
    current_action = get_action(
        current_position, current_velocity, value_function)
    ...
    while True:
        ...
        if time < T:
            ...
            new_postion, new_velocity, reward = step(
                current_position, current_velocity, current_action)
            # choose new action
...
def figure_10_1():
    ...
    axes = [fig.add_subplot(1, len(plot_episodes), i+1, projection='3d')
...
def figure_10_2():
    ...
    for run in range(runs):
        value_functions = [ValueFunction(
            alpha, num_of_tilings) for alpha in alphas]
    ...
    for i in range(0, len(alphas)):
        plt.plot(steps[i], label='alpha = ' +
                 str(alphas[i])+'/'+str(num_of_tilings))
    plt.xlabel('Episode')
...
def figure_10_3():
    ...
    for run in range(runs):
        ...
        for index in range(len(value_functions)):
            for episode in tqdm(range(episodes)):
                step = semi_gradient_n_step_sarsa(
                    value_functions[index], n_steps[index])
                steps[index, episode] += step
